...
if repo.is_dirty(untracked_files=True):
    ...
    # Capture and print the changed and untracked files for user's information
...
if not commit_message:
    commit_message = input('Enter the description of the changes made: ')

# Ask if the user wants to bump the version
bump_decision = input('Do you want to bump the version? (y/n): ')
if bump_decision.lower() == 'y':
...
with open(changelog_file, 'a') as file:
    if bump_decision.lower() == 'y':
        ...
        file.write(f'## [{datetime.datetime.now().strftime("%Y-%m-%d")}] No version bump\n')
...
repo.index.commit(f'Update CHANGELOG.md for version v{new_tag if bump_decision.lower() == "y" else "No version bump"}')
