...
print(f"\n{GREEN_TEXT}You are working in the {BLUE_TEXT}{repo.working_tree_dir}{GREEN_TEXT} repository on the {BLUE_TEXT}{branch_name}{GREEN_TEXT} branch. The latest tag (version) is {BLUE_TEXT}{latest_tag}{RESET_TEXT}\n")
...
if not changed_files and not untracked_files:
    ...
    print(f"\n{GREEN_TEXT}{BOLD_TEXT}{UNDERLINE_TEXT}Changed files:{RESET_TEXT}")
    for i, file in enumerate(changed_files, 1):
        print(f"{i}. {BLUE_TEXT}Modified: {file}{RESET_TEXT}")
    print(f"\nTotal Modified Files: {len(changed_files)}")
    
    print(f"\n{GREEN_TEXT}{BOLD_TEXT}{UNDERLINE_TEXT}Untracked files:{RESET_TEXT}")
    for i, file in enumerate(untracked_files, 1):
        print(f"{i}. {BLUE_TEXT}New file: {file}{RESET_TEXT}")
